...
| `shrinkBudgetMs(int)` | Бюджет спуска по стенным часам — единственная ручка, стоящая детерминизма: докуда дойдёт спуск, зависит от того, как долго исполняется тело |
| `phases(array)` | Какие стадии выполнять (`Phase::Examples`, `Corpus`, `Random`, `Shrink`) — подмножество осознанно меняет покрытие на время |
...
| `PROPERTY_DB` | Путь к каталогу, включающий regression corpus. Не задана — выключено, ничего не пишется |
