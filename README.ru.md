...
а ключ, не являющийся параметром кложуры, —
`Property "…": forAll() covers "vlaue", which is not a parameter of the property`
— с `auto()` и без него, потому что опечатанная запись иначе прошла бы зелёной
в том домене, который есть у настоящего параметра.
...
- Нечитаемый тип (голый `array`, `mixed`, параметр без типа, variadic) —
  ...
  пошире».
- Ключ `forAll()`, не являющийся параметром кложуры, — ошибка, с `auto()` и
  без него; при `auto()` merge-семантика иначе молча заменила бы опечатанную
  запись генератором из сигнатуры.
...
`expectException()` внутри кложуры `check()` работать не может: исполнитель
видит каждый бросок раньше собственного механизма ожиданий PHPUnit, и ожидаемое
исключение фальсифицировало бы property. `throws()` объявляет ожидание там, где
оно проверяется:
...
Семантика, на каждый trial:

- Бросает класс (наследник подходит) — trial **проходит**.
...
Класс, не являющийся `Throwable`, отвергается сразу — опечатанное имя иначе
валило бы каждый прогон без единого слова объяснения.

### Как результаты ложатся на PHPUnit

- Проверка **и есть** assertion: каждый исход засчитывает одну assertion на
  выполняющемся `TestCase`, так что property, тело которой ничего не
  ассертит, никогда не помечается risky — ни на пассе, ни на фальсификации
  (упавшая property отчитывается один раз, как failure). Ничего не
  засчитывает только property, каждый прогон которой был пропущен: такой
  тест пропущен, а не проверен.
...
- `expectException()` не видит исключение тела: движок ловит его как падение
  прогона. Объявляйте ожидание через [`throws()`](#ожидаемые-исключения-throws)
  или проверяйте исключение внутри тела.
...
пока PHPUnit печатает точки прогресса, поэтому каждая строка начинается с
собственного перевода строки (`\n` + строка + `\n`), а не приклеивается к
хвосту `....F..`. Testo-адаптер, у которого строки прогресса нет, печатает те
же строки без ведущего перевода. Потоки и точный формат — диагностика для
человека у терминала, не контракт: grep'ать стоит по префиксу
`Property "<name>"`, а не по смещению в байтах.
